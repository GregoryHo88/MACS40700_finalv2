...
# Check and convert GEOID to character if it's not already
merged_data$GEOID <- as.character(merged_data$GEOID)

# Assuming merged_data is properly set as an sf object
neighbor_list <- st_touches(merged_data, sparse = FALSE)

# Now, each row in neighbor_list corresponds to a GeoID and contains TRUE/FALSE
# for each other GeoID indicating whether it is a neighbor
neighbor_indices <- lapply(1:nrow(merged_data), function(i) {
  which(neighbor_list[i, ])
})

# Calculate neighbor averages efficiently
factor_names <- c(
  "poverty_rate",
  "Education and Health", 
  "Employment and Shelter", 
...
for (factor_name in factor_names) {
  ...
  # Add the new column for neighbor average
